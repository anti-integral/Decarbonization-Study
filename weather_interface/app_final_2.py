...
if pages=="Model Creation":
    ...
    if model=="XGBoost":
        ...
        def make_forecast(df, target):
            ...
            steps = df[df.year.isin([2019, 2020, 2021, 2022])].shape[0]
        ...
        def make_future_forecast(df, learning_rate, n_estimators, target):
            ...
            d = df[df.year.isin([2017, 2018, 2019, 2020, 2021, 2022])].reset_index(drop=True)
    ...
    if model=="Prophet":
        ...
        def make_val_forecast(target):
            ...
            train = df[~df.year.isin([2019, 2020, 2021, 2022])].drop("year", axis=1).reset_index(drop=True)
            test = df[df.year.isin([2019, 2020, 2021, 2022])].drop("year", axis=1).reset_index(drop=True)
    ...
    if model=="Average":
        ...
        def make_forecast(df, target):
            ...
            """
            # Seperate train and test data
            steps = df[df.year.isin([2017, 2018, 2019, 2020, 2021, 2022])].shape[0]
            ...
            data_test = train_test[train_test.year.isin([2017, 2018, 2019, 2020, 2021, 2022])]
            ...
            # final = pd.DataFrame()
            final[target] = df[df["year"].isin([2017, 2018, 2019, 2020, 2021, 2022])][target]
        ...
        def make_future_forecast(df, target):
            # Make future forecast
            # Create forecast data
            d = df[df.year.isin([2017, 2018, 2019, 2020, 2021, 2022])]
            ...
            if target=="temp_diff":
                future["pred"] = df[df.year.isin([2017, 2018, 2019, 2020, 2021, 2022])][target].reset_index(drop=True).values
        ...
        if st.checkbox("Make Future Forecast"):
            ...
            if st.checkbox("Plot Future Avg"):
                if future_target=="temp_diff":
                    train_with_avg = mdf[mdf.year.isin([2017, 2018, 2019, 2020, 2021, 2022])]\
...
if pages=="Use Trained Model":
    ...
    if trained_model=="Average":
        ...
        if target=="temp_diff":
            ...
            future = pd.DataFrame()
            future["pred"] = data[data.year.isin([2017, 2018, 2019, 2020, 2021, 2022])][target].reset_index(drop=True).values
