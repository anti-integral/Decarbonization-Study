...
if pages=="Model Creation":
    ...
    if model=="XGBoost":
        ...
        def make_forecast(df, target):
            ...
            steps = df[df.year.isin([2020, 2021, 2022])].shape[0]
        ...
        def make_future_forecast(df, learning_rate, n_estimators, target):
            ...
            d = df[df.year.isin([2018, 2019, 2020, 2021, 2022])].reset_index(drop=True)
            forecast_data = pd.DataFrame(pd.date_range(start="2023-01-01 00:00:00", end="2025-12-31 23:00:00", freq="H"), columns=["datetime"])
            ...
            if target=="temperature":
                ...
                future["datetime"] = pd.date_range(start="2023-01-01 00:00:00", end="2025-12-31 23:00:00", freq="H")
    ...
    if model=="Prophet":
        ...
        def make_val_forecast(target):
            ...
            train = df[~df.year.isin([2020, 2021, 2022])].drop("year", axis=1).reset_index(drop=True)
            test = df[df.year.isin([2020, 2021, 2022])].drop("year", axis=1).reset_index(drop=True)
        ...
        def make_future_forecast(target):
            ...
            model = Prophet()
            model.fit(train)
            future_df = model.make_future_dataframe(periods=26304, freq='H', include_history=False)
    ...
    if model=="Average":
        ...
        def make_forecast(df, target):
            ...
            """
            # Seperate train and test data
            steps = df[df.year.isin([2018, 2019, 2020, 2021, 2022])].shape[0]
            train_year = df[df.year.isin([2013, 2014, 2015, 2016, 2017])]
                
            data_train = train_year
            data_test  = df[-steps:]
            
            train_test = pd.concat([data_train, data_test])
            data_train = train_test[train_test.year.isin([2013, 2014, 2015, 2016, 2017])]
            data_test = train_test[train_test.year.isin([2018, 2019, 2020, 2021, 2022])]
            ...
            # final = pd.DataFrame()
            final[target] = df[df["year"].isin([2018, 2019, 2020, 2021, 2022])][target]

            if target=="temp_diff":
                final["pred"] = train_test[train_test.year.isin([2013, 2014, 2015, 2016, 2017])][target].values
        ...
        def make_future_forecast(df, target):
            # Make future forecast
            # Create forecast data
            d = df[df.year.isin([2018, 2019, 2020, 2021, 2022])]
            forecast_data = pd.DataFrame(pd.date_range(start="2023-01-01 00:00:00", end="2027-12-31 23:00:00", freq="H"), columns=["datetime"])
            forecast_data["year"] = forecast_data.datetime.dt.year
            forecast_data["month"] = forecast_data.datetime.dt.month
            forecast_data["day"] = forecast_data.datetime.dt.day
            forecast_data["hour"] = forecast_data.datetime.dt.hour
            ...
            forecast_data = forecast_data.query("is_forecast==1").drop("is_forecast", axis=1)
            forecast_data["datetime"] = pd.date_range(start="2023-01-01 00:00:00", end="2027-12-31 23:00:00", freq="H")
            forecast_data = forecast_data.set_index("datetime")
            ...
            f.fit(y=d[target], exog=d[["month", "year", "hour"]])
            future = pd.DataFrame(f.predict(steps=forecast_data.shape[0], exog=forecast_data[["month", "year", "hour"]]).values, columns=["pred"])
            future["datetime"] = pd.date_range(start="2023-01-01 00:00:00", end="2027-12-31 23:00:00", freq="H")
            if target=="temp_diff":
                future["pred"] = df[df.year.isin([2018, 2019, 2020, 2021, 2022])][target].reset_index(drop=True).values
        ...
        if st.checkbox("Make Future Forecast"):
            ...
            if st.checkbox("Plot Future Avg"):
                if future_target=="temp_diff":
                    train_with_avg = mdf[mdf.year.isin([2018, 2019, 2020, 2021, 2022])]\
...
if pages=="Use Trained Model":
    ...
    exog = pd.DataFrame(pd.date_range(start="2023-01-01 00:00:00", end="2025-12-31 23:00:00", freq="H"), columns=["datetime"])
    ...
    if trained_model=="Average":
        ...
        new_df = pd.DataFrame(pd.date_range(start="2023-01-01 00:00:00", end="2027-12-31 23:00:00", freq="H"), columns=["datetime"])
        ...
        if target=="temp_diff":
            ...
            future = pd.DataFrame()
            future["pred"] = data[data.year.isin([2018, 2019, 2020, 2021, 2022])][target].reset_index(drop=True).values
